...
Dataset layout (same as lung/extract_features.py):
    ml/datasets/icbhi/
        {recording}.wav
...
def extract_features(window: np.ndarray, sr: int) -> np.ndarray:
    ...
    fmin=50 Hz keeps all 6 contrast bands within the 2 kHz Nyquist of the piezo ADC.
    """
    ...
    contrast = np.mean(librosa.feature.spectral_contrast(y=window, sr=sr, n_bands=6, fmin=50.0), axis=1)
